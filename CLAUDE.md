...
│   ├── create/          # Factory functions: source, of, fromArray, fromPromise, interval, timer
│   ├── operators/       # Pipe operators: map, filter, scan, debounceTime, throttleTime, switchMap, etc.
│   ├── predefined/      # Higher-level operators: pluck, skip, take, mapOptional, etc.
...
- **`ts-data-forge`** — The only runtime dependency; provides utility types and functions (`Result`, etc.)
- **`ts-repo-utils`** — Dev tooling for monorepo management (build orchestration, index generation, etc.)
- **`ts-codemod-lib`** — Codemods for enforcing `as const` and `readonly` patterns
