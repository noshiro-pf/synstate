...
const embedJsdocDescriptions = async (): Promise<void> => {
  for (const [slug, srcPath, funcName] of jsdocMappings) {
    ...
    const mdPath = path.resolve(refDir, `${slug}.md`);
    ...
    if (replaced === 'no-placeholder') {
      console.warn(`⚠ Placeholder not found in ${slug}.md`);

      continue;
    }

    if (replaced.result === markdown) {
      console.log(`· Already up to date: ${slug}.md`);
    ...
    console.log(`✓ Embedded JSDoc description for ${funcName} → ${slug}.md`);
  }
};
